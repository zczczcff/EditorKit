...
// ActionInvoker - 前向声明（实际定义在 ActionResult 之后）
...
// ActionInvoker - 动作调用器，用于高效执行动作（避免哈希查找）
// 此类提供直接执行路径，无需每次执行时进行哈希查找
template<typename KeyType, typename... Args>
class ActionInvoker
{
private:
    ...
    // 允许 ActionSystem 构造 ActionInvoker
    template<typename KT, bool AO, typename H, typename KE>
    friend class ActionSystem;

    // 私有构造函数，供 ActionSystem 使用
